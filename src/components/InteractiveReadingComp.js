...
function InteractiveReadingComp() {
  ...
  // highlight actual y si ya tiene respuesta
  const currentHighlight = highlightQs[highlightIdx];
  const currentHighlightAnswer =
    currentHighlight && currentHighlight.idx !== undefined
      ? answers[currentHighlight.idx]
      : undefined;

  // deshabilitar Next cuando estamos en highlight y aún no hay selección
  const isNextDisabled =
    passageStage === highlightStage && hasHighlight && !currentHighlightAnswer;

  // handler general para Next (abajo a la derecha)
  const handleNext = () => {
    // Si estamos en etapa de highlight, primero avanzamos entre los highlights
    if (highlightStage !== undefined && passageStage === highlightStage && hasHighlight) {
      if (highlightIdx < highlightQs.length - 1) {
        setHighlightIdx(highlightIdx + 1);
        return; // no pasamos aún al siguiente tipo
      }
      // si ya era el último highlight, reseteamos índice y luego pasamos al siguiente tipo
      setHighlightIdx(0);
    }

    // avanzar al siguiente tipo de pregunta (sentence -> highlight -> identify -> title)
    if (passageStage < lastStage) {
      setPassageStage(passageStage + 1);
    }
  };
  ...
  useEffect(() => {
    fetch("/dataInteractiveReading.json")
      ...
      .catch((err) =>
        console.error("Failed to load interactive reading data", err)
      );
  }, []);
  ...
  const start = () => {
    ...
    const candidates = pool.filter((item) => {
      ...
      if (!item.questions.every((q) => q.type === "CompleteTheSentence"))
        return false;
  ...
  useEffect(() => {
    ...
    if (passageStage !== highlightStage) return;
    const handler = (ev) => {
      try {
        ...
        if (
          !anchor ||
          !passageRef.current ||
          !passageRef.current.contains(anchor)
        )
          return;
        const text = sel.toString().trim();
        if (!text) return;
        const idx = highlightQs[highlightIdx]?.idx;
        if (idx === undefined) return;
        // store the selected text as the answer for the current highlight question
        choose(idx, text);
        sel.removeAllRanges();
      } catch (err) {
    ...
    return () => {
      el && el.removeEventListener("mouseup", handler);
    };
  }, [highlightQs, viewMode, exercise, passageStage, highlightIdx, highlightStage]);
          ...
          <div className="mb-4 flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedTimeSeconds(Number(e.target.value))
                }
                className="text-black px-2 py-1 rounded"
              >
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="bg-[#1f1f1f] rounded p-6 max-h-[70vh] overflow-auto border border-gray-700">
                      ...
                      {selectBestQs.map(({ q, idx }) => {
                        ...
                        const cardClasses = `p-3 rounded border ${
                          "border-blue-400 bg-gray-900 text-gray-200"
                        }`;
                        return (
                          <div
                            key={`mc-preview-${idx}`}
                            className={`${cardClasses} mt-4`}
                          >
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    >
                      ...
                      {selectBestQs.map(({ q, idx }) => {
                        ...
                        const modelText =
                          correctIdx !== undefined ? q.choices[correctIdx] : "";
                        const selIdx = answers[idx];
                        const selectedText =
                          selIdx !== undefined ? q.choices[selIdx] : null;
                        let displayText = "";

                        if (passageStage === 1) {
                          // Primera etapa: mostrar lo que eligió el usuario (preview de su respuesta)
                          displayText = selectedText || "";
                        } else {
                          // Etapas siguientes (highlight, identify, title): mostrar siempre la correcta
                        ...
                        if (displayText) {
                          ...
                          middleArea = (
                            <div className="w-full min-h-[56px] mt-2 rounded border border-gray-700 bg-transparent" />
                          );
                        }
                        return (
                    ...
                    <div className="bg-[#111111] rounded p-6 max-h-[70vh] overflow-auto border border-gray-700 flex flex-col">
                      {passageStage === 1 && (
                        <>
                          <h3 className="text-lg font-semibold text-gray-200 mb-3">
                            Choose the best sentence to complete the passage
                          </h3>
                          <div className="space-y-4 flex-1 overflow-auto">
                            {selectBestQs.length === 0 ? (
                              <div className="text-gray-400">
                                No sentence-choice questions for this exercise.
                              </div>
                            ) : (
                              selectBestQs.map(({ q, idx }) => (
                                <div
                      ...
                      {highlightStage !== undefined &&
                        passageStage === highlightStage &&
                        hasHighlight && (
                          <div className="mt-4">
                            <h3 className="text-lg font-semibold text-gray-200 mb-2">
                              Highlight text in the passage to answer the
                              question below
                            </h3>
                            {(() => {
                              const { q, idx } =
                                highlightQs[highlightIdx] || {};
                              if (q === undefined || idx === undefined)
                                return null;
                              return (
                                <div
                                  key={`highlight-${idx}`}
                                ...
                                >
                                  <div className="text-sm text-gray-300 mb-2">
                                    {q.prompt ||
                                      "Select the text in the passage"}
                                  </div>
                                  <div className="p-4 rounded border border-gray-700 bg-[#0b0b0b] text-gray-200 min-h-[72px]">
                                    {answers[idx] ? (
                                      ...
                                      <div className="text-gray-500">
                                        Select text in the passage above
                                      </div>
                                    )}
                                  </div>
                      ...
                      {identifyStage !== undefined &&
                        passageStage === identifyStage &&
                        hasIdentify && (
                          <div className="mt-4">
                            <h3 className="text-lg font-semibold text-gray-200 mb-3">
                            ...
                            <div className="space-y-4 flex-1 overflow-auto">
                              {identifyQs.length === 0 ? (
                                <div className="text-gray-400">
                                  No idea-identification questions for this
                                  exercise.
                                </div>
                              ) : (
                                identifyQs.map(({ q, idx }) => (
                                  <div
                                    key={`idea-${idx}`}
                                    className="bg-[#0f0f0f] border border-gray-700 rounded p-3"
                                  >
                      ...
                      {titleStage !== undefined &&
                        passageStage === titleStage &&
                        hasTitle && (
                          <div className="mt-4">
                            <h3 className="text-lg font-semibold text-gray-200 mb-3">
                            ...
                            <div className="space-y-4 flex-1 overflow-auto">
                              {titleQs.length === 0 ? (
                                <div className="text-gray-400">
                                  No title questions for this exercise.
                                </div>
                              ) : (
                                titleQs.map(({ q, idx }) => (
                                  <div
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-gray-300">
                        Final Score: {score} / {exercise.questions.length} (
                        {percent}%)
                      </div>
                    </div>
                  </div>
